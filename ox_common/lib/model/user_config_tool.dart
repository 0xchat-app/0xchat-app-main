...
class UserConfigTool{
  ...
  static Future<void> saveSettingToDB() async {
    ...
    if (settingsMap.isNotEmpty) {
      UserDB? currentUser = Account.sharedInstance.me;
      if (currentUser != null) {
  ...
  static Future<void> updateSettingFromDB(String? settings) async {
    ...
    List<StorageSettingKey> settingKeyList = StorageSettingKey.values;
    UserDB? currentUser = OXUserInfoManager.sharedInstance.currentUserInfo;
    if (currentUser != null){
  ...
  static Future<void> saveUser(UserDB userDB) async {
    LogUtil.e('Michael:---saveUser--userDB =${userDB.toMap()}');
