...
extension Ex on Decimal {
  /// Precision processing (without rounded)
  String formatNum(int position) {
    Decimal number = this;
    String value = number.toString();
    bool negative = false;
    // Be negative
    if (value.contains("-") && number <= Decimal.zero) {
      negative = true;
    }

    // Less than zero and not negative
    if (number <= Decimal.zero && !negative) {
    ...
    // Negative numbers are inverted first, and the return value is added to the negative sign
    if (negative) {
      number = -number;
    }

    if ((number.toString().length - number.toString().lastIndexOf(".") - 1) < position) {
      final value = number
      ...
      if (position == 0) {
        final value = number
            .toString()
            .substring(0, number.toString().lastIndexOf(".") + position)
            .toString();
        return negative ? "-$value" : value;
      }
      final value = number
