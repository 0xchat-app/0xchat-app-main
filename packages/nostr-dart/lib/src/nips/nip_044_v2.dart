...
/// https://github.com/paulmillr/nip44/tree/main
class Nip44v2 {
  ...
  static Map<String, Uint8List> getMessageKeys(Uint8List conversationKey, Uint8List nonce) {
  ...
  static Uint8List writeU16BE(int num) {
    if (num < 1 || num > 65535) throw Exception('Invalid plaintext size: must be between 1 and 65535 bytes');
  ...
  static String unpad(Uint8List padded) {
    var unpaddedLen = ByteData.sublistView(padded, 0, 2).getUint16(0, Endian.big);
    var unpadded = padded.sublist(2, 2 + unpaddedLen);
    if (unpaddedLen < 1 || unpaddedLen > 65535 || unpadded.length != unpaddedLen || padded.length != 2 + calcPaddedLen(unpaddedLen)) {
  ...
  static Uint8List hmacAad(Uint8List key, Uint8List message, Uint8List aad) {
    if (aad.length != 32) throw Exception('AAD associated data must be 32 bytes');
  ...
  static Future<Uint8List> chacha20Encrypt(Uint8List key, Uint8List nonce, Uint8List data) async {
    final algorithm = cryptography.Chacha20(macAlgorithm: cryptography.MacAlgorithm.empty);
    // Encrypt
  ...
  static Future<Uint8List> chacha20Decrypt(Uint8List key, Uint8List nonce, Uint8List ciphertext) async {
    final algorithm = cryptography.Chacha20(macAlgorithm: cryptography.MacAlgorithm.empty);
    cryptography.SecretBox secretBox =
    cryptography.SecretBox(ciphertext, nonce: nonce, mac: cryptography.Mac.empty);
    // Encrypt
  ...
  static Future<String> encrypt(String plaintext, Uint8List conversationKey, [Uint8List? nonce]) async {
    nonce ??= randomBytes(32);
    var keys = getMessageKeys(conversationKey, nonce);
    var padded = pad(plaintext);
    var ciphertext = await chacha20Encrypt(keys['chacha_key']!, keys['chacha_nonce']!, padded);
  ...
  static Future<String> decrypt(String payload, Uint8List conversationKey) async {
    ...
    var calculatedMac = hmacAad(keys['hmac_key']!, payloadData['ciphertext']!, payloadData['nonce']!);
    if (!listEquals(calculatedMac, payloadData['mac']!)) {
      throw Exception('Invalid MAC');
    }
    var padded = await chacha20Decrypt(keys['chacha_key']!, keys['chacha_nonce']!, payloadData['ciphertext']!);
    return unpad(padded);
  }
